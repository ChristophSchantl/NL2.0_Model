...
st.set_page_config(page_title="SHI – STOCK CHECK / Signal-basierte Strategie", layout="wide")
...
THRESH   = st.sidebar.number_input("Threshold für Target", 0.0, 0.1, 0.046, step=0.005, format="%.3f")
...
def _band_mask(strikes: pd.Series, atm: float, band: float) -> pd.Series:
    if not np.isfinite(atm): return strikes == False
...
# ─────────────────────────────────────────────────────────────
# Features, Training & Backtest (Walk-Forward-fähig)
# ─────────────────────────────────────────────────────────────
def make_features_and_train(
    ...
    df_bt, trades = backtest_next_open(
        ...
        INIT_CAP, POS_FRAC, min_hold_days=int(min_hold_days), cooldown_days=int(cooldown_days)
    )
...
with st.expander("Optimizer (Random Search mit Walk-Forward-Light)", expanded=False):
    ...
    if st.button("🔎 Suche starten", type="primary", use_container_width=True):
        ...
        feasible_tickers = [tk for tk, df in price_map.items() if df is not None and len(df) >= 80]
            ...
            with st.expander(f"Trades (Next Open) für {ticker}", expanded=False):
                ...
                if not trades_df.empty:
                    ...
                    df_tr["CumPnL"] = (
                        ...
                        .cumsum().fillna(method="ffill").fillna(0)
                    )
...
if results:
    ...
    styled = (
        ...
        .applymap(lambda v: "font-weight: bold;" if isinstance(v,(int,float)) else "", subset=pd.IndexSlice[:,["Sharpe-Ratio","Sortino-Ratio"]])
    ...
    NAME_OVERRIDES = {"QBTS":"D-Wave Quantum Inc.","NOG":"Northern Oil and Gas, Inc.","LUMN":"Lumen Technologies, Inc."}
    ...
    for ticker, trades in all_trades.items():
        if trades and trades[-1]["Typ"] == "Entry":
            ...
            open_positions.append({
                "Ticker": ticker, "Name": name, "Entry Date": entry_ts,
    ...
    if open_positions:
        ...
        styled_open = open_df_display.style.format({
            "Entry Price": "{:.2f}", "Current Prob.": "{:.4f}", "Unrealized PnL (€)": "{:.2f}",
        })
        show_styled_or_plain(open_df_display, styled_open)
        st.download_button(
            "Offene Positionen als CSV", to_csv_eu(open_df),
    ...
    if not rt_df.empty:
        ...
        if len(price_series) < 2:
            ...
            if rets.shape[1] < 2 or len(common_rows) < min_obs:
                ...
                fig_corr.update_layout(height=560, margin=dict(t=40, l=40, r=30, b=40), coloraxis_colorbar=dict(title="ρ"))
        ...
        if corr is not None and corr.shape[0] >= 2:
            ...
            w = np.full(N, 1.0 / N)
            ip_corr_raw = float(w @ corr.values @ w)
