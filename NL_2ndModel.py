...
if pd.to_datetime(END_DATE) < pd.to_datetime(START_DATE):
    st.sidebar.error("End Date muss >= Start Date sein.")
    st.stop()
...
def compute_performance(df_bt: pd.DataFrame, trades: List[dict], init_cap: float) -> dict:
    ...
    vol_ann = rets.std(ddof=0) * sqrt(TRADING_DAYS) * 100
    sharpe = (rets.mean() * sqrt(TRADING_DAYS)) / (rets.std(ddof=0) + 1e-12)
    ...
    if len(price_series) < 2:
        ...
        if rets2.empty:
            ...
            if port_ret.empty:
                ...
                ann_return = (1.0 + port_ret).prod() ** (TRADING_DAYS / len(port_ret)) - 1.0
                ann_vol = port_ret.std(ddof=0) * np.sqrt(TRADING_DAYS)
                sharpe = (port_ret.mean() / (port_ret.std(ddof=0) + 1e-12)) * np.sqrt(TRADING_DAYS)
                ...
                if open_df.empty:
                    ...
                    if not rows_fc:
                        ...
                        sort_col = f"E[r {int(FORECAST_DAYS)}d]"
                        fc_df = fc_df.sort_values(by=sort_col, ascending=False)
                        st.dataframe(fc_df, use_container_width=True)

                        exp_rets = fc_df[sort_col].astype(float).dropna()
                        ...
                        if len(exp_rets) < 1:
                            ...
                            # Cov aus Close-Returns der offenen Positionen (Scaling auf Horizon)
                            ...
                            w_used = out["weights_used"]
                            exp_port_ret = float(np.dot(w_used, exp_rets.values))
