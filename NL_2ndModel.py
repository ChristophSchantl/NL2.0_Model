...
# ─────────────────────────────────────────────────────────────
THEME = {
    "bg":         "#0C0F0A",   # near-black with green tinge
    "bg_card":    "#121712",
    "bg_panel":   "#161D14",
    "accent1":    "#4ADE80",   # vivid green — primary signal
    "accent2":    "#FB923C",   # amber — warning / exit
    "accent3":    "#38BDF8",   # sky — info / hold
    "accent4":    "#A78BFA",   # violet — probability
    "red":        "#F87171",
    "green":      "#4ADE80",
    "text":       "#E8F0E9",
    "muted":      "#6B7C6E",
...
PLOTLY_BASE = dict(
    ...
    font=dict(family="'JetBrains Mono', 'Fira Code', monospace", color=THEME["muted"], size=10),
    xaxis=dict(
        gridcolor=THEME["grid"], gridwidth=1,
        showline=True, linecolor=THEME["border"],
    ...
    hoverlabel=dict(
        bgcolor="#0E1A0E", font_color=THEME["text"],
    ...
    modebar=dict(bgcolor="rgba(0,0,0,0)", color=THEME["muted"], activecolor=THEME["accent1"]),
)
...
[data-testid="metric-container"]:hover { border-color: var(--g1) !important; }
...
details[open] summary { color: var(--g1) !important; }
...
.stDataFrame tbody tr:hover { background: rgba(74,222,128,0.04) !important; }
...
.stButton > button:hover { background: var(--g1) !important; color: #0C0F0A !important; }
.stButton > button[data-testid*="primary"] {
  background: var(--g1) !important; color: #0C0F0A !important; font-weight: 700 !important;
}
...
/* Progress */
.stProgress > div > div { background: var(--g1) !important; }
...
.stTabs [aria-selected="true"] {
  color: var(--g1) !important; border-bottom-color: var(--g1) !important;
}

/* Custom classes */
.nexus-header {
  font-family: 'Syne', sans-serif; font-weight: 800;
  font-size: 32px; color: var(--text); letter-spacing: -0.01em;
  line-height: 1.1;
}
.nexus-sub {
...
.section-bar-dot {
  width: 5px; height: 5px; border-radius: 50%; background: var(--g1);
  box-shadow: 0 0 8px var(--g1);
}
.ticker-header {
  font-family: 'JetBrains Mono', monospace; font-weight: 700;
  font-size: 13px; color: var(--g1); text-transform: uppercase; letter-spacing: 0.1em;
}
...
.kpi-box:hover { border-color: var(--g1); }
.kpi-label {
  font-family: 'JetBrains Mono', monospace; font-size: 9px;
  color: var(--muted); text-transform: uppercase; letter-spacing: 0.12em; margin-bottom: 5px;
}
.kpi-value {
  font-family: 'JetBrains Mono', monospace; font-size: 18px; font-weight: 700;
  color: var(--text);
}
.kpi-pos .kpi-value { color: var(--g1); }
.kpi-neg .kpi-value { color: var(--red); }
.kpi-info .kpi-value { color: var(--a3); }
.kpi-warn .kpi-value { color: var(--a2); }
...
.sig-enter { background: rgba(74,222,128,0.12); color: #4ADE80; border: 1px solid rgba(74,222,128,0.3); }
.sig-exit  { background: rgba(248,113,113,0.12); color: #F87171; border: 1px solid rgba(248,113,113,0.3); }
.sig-hold  { background: rgba(107,124,110,0.12); color: #6B7C6E; border: 1px solid rgba(107,124,110,0.25); }
</style>
...
def kpi_row(items: list):
    ...
    parts = "".join(f'<div class="kpi-box {cls}"><div class="kpi-label">{lbl}</div><div class="kpi-value">{val}</div></div>'
...
with st.sidebar:
    st.markdown("""
    <div style="padding:14px 0 22px;border-bottom:1px solid #1F2B1F;margin-bottom:18px;">
      <div style="font-family:'Syne',sans-serif;font-weight:800;font-size:20px;color:#E8F0E9;letter-spacing:-0.01em;">NEXUS</div>
      <div style="font-family:'JetBrains Mono',monospace;font-size:10px;color:#4ADE80;letter-spacing:0.14em;margin-top:2px;">2ND AI MODEL · v2.0</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if EXIT_PROB >= ENTRY_PROB:
        st.error("⚠ Exit < Entry required"); st.stop()
...
def compute_performance(df_bt, trades, init_cap):
    ...
    return {
        "Strategy Net (%)":round(float(net_ret),2),"Strategy Gross (%)":round(float((df_bt["Equity_Gross"].iloc[-1]/init_cap-1)*100),2),
        ...
        "Max Drawdown (%)":round(float(max_dd),2),"Calmar-Ratio":round(float(calmar),2) if np.isfinite(calmar) else np.nan,
...
def make_features_and_train(df, lookback, horizon, threshold, model_params,
    ...
    if hist["Target"].nunique() < 2:
        ...
        p = pipe(); p.fit(hist[X_cols].values, hist["Target"].values)
        ...
        for t in range(mt, len(feat)):
            ...
            p = pipe(); p.fit(tr[X_cols].values, tr["Target"].values)
            probs[t] = p.predict_proba(feat[X_cols].iloc[[t]].values)[0,1]
        feat["SignalProb"] = pd.Series(probs, index=feat.index).ffill().fillna(0.5)

    df_bt, trades = backtest_next_open(feat.iloc[:-1], entry_prob, exit_prob, COMMISSION,
...
def chart_price_signal(feat, trades, ticker):
    """Candlestick + Signal-Prob als Subpanel + Entry/Exit Marker."""
    ...
    if all(c in feat.columns for c in ["Open","High","Low","Close"]):
        fig.add_trace(go.Candlestick(
            ...
            increasing=dict(line=dict(color=THEME["green"],width=1), fillcolor="rgba(74,222,128,0.25)"),
            decreasing=dict(line=dict(color=THEME["red"],  width=1), fillcolor="rgba(248,113,113,0.25)"),
        ), row=1, col=1)
    else:
    ...
    if not tdf.empty:
        ...
        for typ, sym, col, side in [("Entry","triangle-up",THEME["green"], 1),
                                     ("Exit","triangle-down",THEME["red"], -1)]:
            sub = tdf[tdf["Typ"]==typ]
            if not sub.empty:
    ...
    fig.update_layout(
        title=dict(text=f"<b>{ticker}</b>  ·  Preis & Signal", font=dict(size=12, color=THEME["muted"])),
...
def chart_equity(df_bt, ticker, init_cap):
    ...
    fig.update_layout(
        title=dict(text=f"<b>{ticker}</b>  ·  Equity & Drawdown", font=dict(size=12, color=THEME["muted"])),
...
def chart_intraday(intra, ticker, tdf, chart_type, interval):
    ...
    if chart_type == "Candlestick (OHLC)":
        fig.add_trace(go.Candlestick(
            ...
            increasing=dict(line=dict(color=THEME["green"],width=1), fillcolor="rgba(74,222,128,0.2)"),
            decreasing=dict(line=dict(color=THEME["red"],  width=1), fillcolor="rgba(248,113,113,0.2)"),
        ))
    else:
    ...
    for _, ds in intra.groupby(intra.index.normalize()):
        fig.add_vline(x=ds.index.min(), line_width=1, line_dash="dot",
                      line_color=THEME["border"], opacity=0.5)
    ...
    fig.update_layout(
        title=dict(text=f"<b>{ticker}</b>  ·  Intraday 5d ({interval})", font=dict(size=12, color=THEME["muted"])),
        xaxis_rangeslider_visible=False,
    )
    return fig
...
def chart_corr(corr):
    fig = go.Figure(go.Heatmap(
        ...
        colorscale=[
            [0.0, THEME["red"]], [0.5, THEME["bg_panel"]], [1.0, THEME["accent1"]]
        ],
    ...
    fig.update_layout(
        title=dict(text="Korrelationsmatrix", font=dict(size=12, color=THEME["muted"])),
...
def chart_portfolio_nav(nav):
    ...
    fig.update_layout(
        title=dict(text="Portfolio NAV · Equal-Weight", font=dict(size=12, color=THEME["muted"])),
        yaxis=dict(title="NAV (€)", tickformat=",.0f"),
        showlegend=False,
    )
    return fig
...
def chart_mc_histogram(port_rets, q05, q50, q95, forecast_days, sims):
    ...
    fig.add_trace(go.Histogram(
        ...
        marker=dict(
            color=["rgba(74,222,128,0.6)" if v >= 0 else "rgba(248,113,113,0.6)"
                   for v in port_rets],
    ...
    for val, lbl, col in [(q05*100,"5%",THEME["red"]), (q50*100,"50%",THEME["accent3"]),
                           (q95*100,"95%",THEME["green"])]:
        fig.add_vline(x=val, line_dash="dash", line_color=col, line_width=1.5, opacity=0.8,
                      annotation_text=f"  {lbl}: {val:.2f}%",
                      annotation_font=dict(color=col, size=9, family="'JetBrains Mono', monospace"),
...
def chart_histogram(data, xlabel, title, bins):
    ...
    fig = go.Figure(go.Histogram(
        x=data, nbinsx=bins,
        marker=dict(
            color=["rgba(74,222,128,0.55)" if v >= 0 else "rgba(248,113,113,0.55)" for v in data],
            line=dict(color="rgba(0,0,0,0)", width=0)
        ),
        showlegend=False,
    ...
    if np.isfinite(mean_v):
        fig.add_vline(x=mean_v, line_dash="dot", line_color=THEME["accent2"], line_width=1.5,
                      annotation_text=f"  Ø {mean_v:.2f}",
                      annotation_font=dict(color=THEME["accent2"],size=9,
                                           family="'JetBrains Mono', monospace"),
                      annotation_position="top right")
    _apply_theme(fig, 340)
    fig.update_layout(title=dict(text=title, font=dict(size=12, color=THEME["muted"])),
...
with st.expander("⚙ Random-Search Optimizer  ·  Walk-Forward Light", expanded=False):
    ...
    if st.button("🚀 Suche starten", type="primary", use_container_width=True):
        ...
        for trial in range(int(n_trials)):
            p = dict(lookback=rng_opt.randrange(lb_lo,lb_hi+1,5), horizon=rng_opt.randrange(hz_lo,hz_hi+1),
                     thresh=rng_opt.uniform(thr_lo,thr_hi), entry=rng_opt.uniform(en_lo,en_hi),
            ...
            for tk, df0 in feasible.items():
                ...
                try:
                    ...
                    pp = Pipeline([("i",SimpleImputer(strategy="median")),
                    ...
                    df_bt_o, tr_o = backtest_next_open(feat_o.iloc[:-1], float(p["entry"]), float(p["exit"]),
        ...
        if not rows_o:
            ...
            st.download_button("⬇ Optimizer-Ergebnisse", to_csv_eu(df_res_o),
...
# ─────────────────────────────────────────────────────────────
st.markdown("""
<div style="padding:28px 0 8px;border-bottom:1px solid #1F2B1F;margin-bottom:28px;">
...
for ticker in TICKERS:
    ...
    with st.expander(f"⚡  {ticker}  ·  {name}", expanded=False):
        try:
            ...
            st.caption(f"🕐 {ts.strftime('%Y-%m-%d %H:%M %Z')}{sfx}  ·  "
            ...
            results.append(metrics); all_trades[ticker]=trades; all_dfs[ticker]=df_bt; all_feat[ticker]=feat
...
if live_forecasts_run:
    ...
    def _board_color(row):
        ...
        if "enter" in a: return [f"background: rgba(74,222,128,0.07)"]*len(row)
        if "exit"  in a: return [f"background: rgba(248,113,113,0.07)"]*len(row)
        return [""]*len(row)
...
if results:
    ...
    def _num_col(v):
        try: return f"color: {THEME['green']}" if float(v)>0 else (f"color: {THEME['red']}" if float(v)<0 else "")
        except: return ""
    def _phase_col(v):
        if str(v)=="Open":  return f"background: rgba(56,189,248,0.12); color: {THEME['accent3']}; border-radius:2px; padding:1px 6px;"
        if str(v)=="Flat":  return f"background: rgba(107,124,110,0.1); color:{THEME['muted']};border-radius:2px;padding:1px 6px;"
        return ""
    ...
    if open_positions:
        ...
        def _upnl_c(v):
            try: return f"color:{THEME['green']}" if float(v)>=0 else f"color:{THEME['red']}"
            except: return ""
        st.dataframe(op_df.style.format({"Entry Price":"{:.2f}","Close":"{:.2f}","Signal Prob":"{:.4f}","uPnL (€)":"{:.2f}"}
...
# Footer
st.markdown(f"""
<div style="margin-top:56px;padding:18px 0;border-top:1px solid {THEME['border']};
text-align:center;color:{THEME['muted']};font-family:'JetBrains Mono',monospace;
